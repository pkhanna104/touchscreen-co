...
# dict(tt=[tt_15_sec.active, tt_30_sec.active, tt_45_sec.active, tt_60_sec.active]))
    ...
    def init(self, stims=None, animal_names_dict=None, tdbt=None, rew_in=None, task_in=None,
        ...
        # rew_all.active, rew_50.active, rew_30.active, rew_easy.active, rew_medium.active, rew_hard.active
        reward_var_opt = [1.0, .5, .33, 'easy', 'medium', 'hard']
        for i, val in enumerate(rew_var['rew_var']):
            if val:
                ...
                elif self.percent_of_trials_rewarded == 1.0:
                    self.percent_of_trials_doubled = 0.0
                elif self.percent_of_trials_rewarded == .5:
                    self.percent_of_trials_doubled = 0.0


                elif self.percent_of_trials_rewarded == 'easy':
                    self.percent_of_trials_doubled = 0.0
                elif self.percent_of_trials_rewarded == 'medium':
                    self.percent_of_trials_doubled = 0.0
                elif self.percent_of_trials_rewarded == 'hard':
                    self.percent_of_trials_doubled = 0.0
...
# what are these?
        ...
        d = dict(animal_name=animal_name, center_target_rad=self.center_target_rad,
            ...
            wrong_target_counter = self.wrong_target_counter,
    ...
    def close_app(self):
        ...
        if self.idle:
            ...
            self.tdbt_param = str(self.tdbt_type)
